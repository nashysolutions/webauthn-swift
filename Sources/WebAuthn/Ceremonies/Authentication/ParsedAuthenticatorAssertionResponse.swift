...
struct ParsedAuthenticatorAssertionResponse {
    ...
    init(from authenticatorAssertionResponse: AuthenticatorAssertionResponse) throws {
        ...
        guard let signature = URLEncodedBase64(bytes: authenticatorAssertionResponse.signature) else {
    ...
    func verify(
        ...
        try credentialPublicKey.verify(signature: Data(signature.bytes), data: signatureBase)
    }
}
